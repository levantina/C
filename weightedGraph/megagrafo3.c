...
void createGraph2D(graph *grafo, int L);
...
int main(int argc, char *argv[]){
  ...
  my_heap *Q;
  vertex ver_temp;
  int i,j,inizio, fine, L,L2, iterazioni,step;
  unsigned int seed;

  if(argc!=5){
    printf("\nERRORE\nUtilizzo: %s <OutputFile> <NumeroVerticiLato> <Iterazioni> <seed>\n", argv[0]);
    exit(1);
  }
  printf("\nCALCOLO DEL CAMMINO MINIMO IN UN GRAFO NON DIRETTO QUADRATO TRA I VERTICI 0 E L^2-1\nValentina Biagini\n");
  /*infl = (FILE*)fopen(argv[1],"r");*/
  outfl = (FILE*)fopen(argv[1], "w");
  ...
  L = (int)atoi(argv[2]);
  iterazioni = (int)atoi(argv[3]);
  seed = (unsigned)atoi(argv[4]);
  srand(seed);
  L2 = L*L;
  fprintf(outfl,"#<Passi> <PesoCammino> (Lato = %d Iterazioni = %d)\n",L,iterazioni);
  ...
  grafo->num_ver = L2;
  grafo->num_edge = 3*(L-1)*(L-1)+(L-1)*2;
  inizio = 0;
  fine = L2-1;
  createGraph2D(grafo, L);
  infl = (FILE*)fopen("Ngrafi.graph","r");
  ...
  ver_temp = grafo->ver[fine];


  while(ver_temp.pos != inizio){
      ver_temp = *grafo->ver[ver_temp.pos].pred;
      step++;
    }
   
    fprintf(outfl,"%d %f\n", step, grafo->ver[fine].dist);
...
void createGraph2D(graph *grafo, int L){
  ...
  out = (FILE*)fopen("Ngrafi.graph","w");
  lato = L;
  lato2 = grafo->num_ver;
  
  fprintf(out, "N = %d M = %d\n", lato2, grafo->num_edge);
